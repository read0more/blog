...
test.describe("카테고리", () => {
  test("/category/React — React 글 2개만 표시되고 사이드바가 active", async ({
    page,
  }) => {
  ...
  test("한글 카테고리 /category/성능 — 1개 글, 사이드바 active", async ({
    page,
  }) => {
  ...
  test("새 카테고리 자동 생성 — 'QA메모'가 사이드바에 나타난다", async ({
    page,
  }) => {
  ...
  test("사이드바 카테고리 색상칩 — 모든 카테고리에 컬러 dot", async ({
    page,
  }) => {
    await page.goto("/");
