...
그래서 줄에서 **위치가 고정**되는 부분(썸네일, 우측 금액)은 골격이니 **slot**으로 컴포넌트가 직접 잡고, **무엇이 들어올지 종류가 다양한** 부분(상품명+옵션 라인인지, 결제 금액 항목 섹션의 라인인지)은 **children**으로 바깥에 넘겼다. 이 과정에서 무엇을 출력할지 바깥에 맡김으로써 추상화가 되어 type을 받을 필요가 없어졌다.
