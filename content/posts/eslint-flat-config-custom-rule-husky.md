...
추가적으로 요즘에는 직접 손으로 코딩하기보다는 AI에게 지시하는 경우가 많은데, AI는 **린트를 강제로 우회**하는 코드를 쓰는 경우가 있었다. 이를 막기 위해 CLAUDE.md에 아래의 내용을 추가했다.
```md
...
위 설정을 마치고 나면 다음이 사람의 의지와 무관하게 강제된다.
...
마지막으로, 도구는 사람이 빼먹지 않을 때만 의미가 있다는 점이다. lint를 갖춰도 강제하지 않으면 AI던 사람이던 결국 외면하게 되는 리스크가 생긴다. git hook으로 강제하고 CLAUDE.md로 AI까지 묶고 나서야 비로소 의지와 무관하게 강제되었다.
